...
class Profile(models.Model):
    ...
    contactnumber = models.CharField(validators=[phone_regex], max_length=10, blank=True)
    type_user = models.CharField(max_length=20, default='student', choices=typeuser)
    CB = (('InformationTechnology', "InformationTechnology"), ('ComputerScience', "ComputerScience"),
          ('InformationScience', "InformationScience"), ('Electronics and Communication', "Electronics and Communication"),
          ('Mechanical', "Mechanical"))
    Branch = models.CharField(choices=CB, max_length=29, default='InformationTechnology')

    def __str__(self):
        return f"{self.collegename} - {self.user.username}"
...
class Complaint(models.Model):
    ...
    def __str__(self):
        return f"{self.get_Type_of_complaint_display()} - {str(self.user)}"
